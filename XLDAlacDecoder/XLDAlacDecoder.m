...
static xldoffset_t getTotalFrames(FILE *fp, int *fixupRequired, off_t *fixupSttsOffset, off_t *fixupStscOffset)
{
	char atom[4];
	...
	stscPos = ftello(fp);
	if(totalALACFrames != totalALACFramesFromStts) {
		if(((totalSamples + 4096) & 0x1ffff) == 0) {
			if(totalALACFrames == totalALACFramesFromStts + 1) {
				if(fixupRequired) *fixupRequired = 1;
				if(fixupSttsOffset) *fixupSttsOffset = sttsPos;
				if(fixupStscOffset) *fixupStscOffset = stscPos;
			}
		}
		goto end;
	}
...
NSString *fixupBadAlacFile(NSString *srcPath, off_t fixupSttsPos, off_t fixupStscPos)
{
	NSString *tmpPath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"XLDAlacDecoder.XXXXXX"];
	char *path = strdup([tmpPath UTF8String]);
	mktemp(path);
	tmpPath = [NSString stringWithUTF8String:path];
	free(path);
	NSFileManager *fm = [NSFileManager defaultManager];
#if MAC_OS_X_VERSION_MIN_REQUIRED < 1050
	[fm copyPath:srcPath toPath:tmpPath handler:nil];
#else
	[fm copyItemAtPath:srcPath toPath:tmpPath error:nil];
#endif
	FILE *fp = fopen([tmpPath UTF8String], "r+b");
	if(!fp) goto fail;
	
	int n, tmp;
	if(fseeko(fp,fixupSttsPos+4,SEEK_SET) != 0) goto fail;
	if(fread(&n,4,1,fp) < 1) goto fail;
	n = OSSwapBigToHostInt32(n);
	if(n>1) if(fseeko(fp,(n-1)*8,SEEK_CUR) != 0) goto fail;
	if(fread(&tmp,4,1,fp) < 1) goto fail;
	tmp = OSSwapBigToHostInt32(tmp);
	tmp++;
	tmp = OSSwapHostToBigInt32(tmp);
	if(fseeko(fp,-4,SEEK_CUR) != 0) goto fail;
	if(fwrite(&tmp,4,1,fp) < 1) goto fail;
	
	if(fseeko(fp,fixupStscPos+4,SEEK_SET) != 0) goto fail;
	if(fread(&n,4,1,fp) < 1) goto fail;
	n = OSSwapBigToHostInt32(n);
	if(fseeko(fp,(n-1)*12+4,SEEK_CUR) != 0) goto fail;
	if(fread(&tmp,4,1,fp) < 1) goto fail;
	tmp = OSSwapBigToHostInt32(tmp);
	tmp++;
	tmp = OSSwapHostToBigInt32(tmp);
...
+ (BOOL)canHandleFile:(char *)path
{
	...
	if(ExtAudioFileOpenURL((CFURLRef)url, &infile) != noErr) return NO;
#endif
...
- (BOOL)openFile:(char *)path
{
...
#else
	NSURL *url = [NSURL fileURLWithPath:[NSString stringWithUTF8String:path]];
	if(ExtAudioFileOpenURL((CFURLRef)url, &file) != noErr) return NO;
#endif
	...
	fixupRequired = 0;
	fixupPath = nil;
	totalFramesFromFile = getTotalFrames(fp, &fixupRequired, &fixupSttsOffset, &fixupStscOffset);
...
- (void)doFixup
{
	if(fixupRequired) {
		fixupRequired = 0;
		fixupPath = fixupBadAlacFile(srcPath, fixupSttsOffset, fixupStscOffset);
		if(fixupPath) {
			[fixupPath retain];
			ExtAudioFileDispose(file);
#if MAC_OS_X_VERSION_MIN_REQUIRED < 1050
			FSRef inputFSRef;
			FSPathMakeRef([fixupPath UTF8String],&inputFSRef,NULL);
			ExtAudioFileOpen(&inputFSRef, &file);
#else
			NSURL *url = [NSURL fileURLWithPath:fixupPath];
			ExtAudioFileOpenURL((CFURLRef)url, &file);
#endif
			size_t size = sizeof(outputFormat);
			ExtAudioFileSetProperty(file, kExtAudioFileProperty_ClientDataFormat, size, &outputFormat);
		}
	}
}
...
- (void)closeFile
{
	...
	if(fixupPath) {
#if MAC_OS_X_VERSION_MIN_REQUIRED < 1050
		[[NSFileManager defaultManager] removeFileAtPath:fixupPath handler:nil];
#else
		[[NSFileManager defaultManager] removeItemAtPath:fixupPath error:nil];
#endif
		[fixupPath release];
		fixupPath = nil;
	}
