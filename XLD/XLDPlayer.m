...
- (id)init
{
	...
	if(floor(NSAppKitVersionNumber) > 1894) {
		[o_positionSlider setControlSize:NSControlSizeMini];
	}
#endif
...
- (void)play
{
	...
	int *buffer_decoder = (int *)malloc(bufferSize);
	...
	free(buffer_decoder);
	if(playing && [decoder error]) {
		[self performSelectorOnMainThread:@selector(stop) withObject:nil waitUntilDone:NO];
	}
	else if(playing) {
		[self performSelectorOnMainThread:@selector(stopAndPlay) withObject:nil waitUntilDone:NO];
	}
