...
- (int)decodeToBuffer:(int *)buffer frames:(int)count
{
	...
	if(result->parent) {
		...
		xldoffset_t startOfPreTrack = [result->validator startOfPreTrack];
		for(i=0;i<count;i++) {
			if(result->next && currentFrame >= startOfPreTrack && currentFrame - startOfPreTrack < 2352) {
				preTrackSamples[(currentFrame - startOfPreTrack)*2] = buffer[i*2];
				preTrackSamples[(currentFrame - startOfPreTrack)*2+1] = buffer[i*2+1];
			}
			if(result->prev && currentFrame < 2352) {
				postTrackSamples[currentFrame*2] = buffer[i*2];
				postTrackSamples[currentFrame*2+1] = buffer[i*2+1];
			}
			currentFrame++;
			if(result->next && currentFrame == startOfPreTrack + 2352) {
				[result->next->validator commitPreTrackSamples:preTrackSamples];
			}
			if(result->prev && currentFrame == 2352) {
				[result->prev->validator commitPostTrackSamples:postTrackSamples];
			}
		}
	}
	if(!testMode) {
