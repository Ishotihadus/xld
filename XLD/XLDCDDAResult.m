...
- (id)initWithTrackNumber:(int)t
{
	...
	for(i=0;i<t+1;i++) {
		...
		results[i].peak = -1;
		if(i>0 && i<t) results[i].next = &results[i+1];
		if(i>1 && i<t+1) results[i].prev = &results[i-1];
	}
	trackNumber = t;
	return self;
}
...
- (void)setTOC:(NSArray *)arr
{
	...
	for(i=0;i<[arr count];i++) {
		...
		if(i == ([arr count]-1)) {
			...
			[results[i+1].validator setStartOfPreTrack:actualLengthArr[i]-2352];
		}
		...
		results[i+1].willSkip = ![(XLDTrack *)[arr objectAtIndex:i] enabled];
	}
...
{
	if(!desc) return;
	[self setDriveStr:[NSString stringWithFormat:@"%s %s (revision %s)",desc->vendor,desc->product,desc->revision]];
	[self setMediaType:[NSString stringWithUTF8String:desc->mediaType]];
}
