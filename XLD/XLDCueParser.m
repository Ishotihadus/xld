...
- (id)initWithDelegate:(id)del
{
	...
	delegate = [del retain];
	if(delegate) {
		decoderCenter = [[delegate decoderCenter] retain];
	}
	return self;
}
...
- (void)dealloc
{
	...
	if(decoderCenter) [decoderCenter release];
	[super dealloc];
}
...
- (NSString *)setTrackData:(NSMutableArray *)tracks forCueFile:(NSString *)file withDecoder:(id)decoder
{
	...
	if(preferredEncoding && preferredEncoding != 0xFFFFFFFF) enc = preferredEncoding;
...
- (NSString *)setTrackData:(NSMutableArray *)tracks andLayout:(XLDDiscLayout *)layout forNonCompliantCueFile:(NSString *)file error:(XLDErr *)error
{
	...
	unsigned char bom[] = {0xEF,0xBB,0xBF};
	unsigned char tmp[3];
	xldoffset_t pendingHTOALength = 0;
	...
	while(fgets_private(cuebuf,512,fp) != NULL) {
		...
		if(!strncasecmp(cuebuf+i,"FILE",4)) {
			...
			if(decoder) { /* 2nd track or later */
				...
				[decoder closeFile];
				decoder = [decoderCenter preferredDecoderForFile:path];
			}
			else { /* 1st track */
				decoder = [decoderCenter preferredDecoderForFile:path];
			}
			...
			if(pendingHTOALength) {
				xldoffset_t gapLength = pendingHTOALength * [decoder samplerate] / 75;
		...
		if(stat == 1) {
			...
			if(!strncasecmp(cuebuf+i,"INDEX",5)) {
				...
				if(decoder) {
					...
					pendingHTOALength = (min*60+sec)*75 + frame;
				}
			}
			else if(!strncasecmp(cuebuf+i,"TITLE",5)) {
				NSString *str = readMetadataFromLineBuf(cuebuf,i+5,enc,YES);
...
- (XLDErr)openNonCompliantCueFile:(NSString *)file
{
	...
	XLDDiscLayout *layout = [[XLDDiscLayout alloc] initWithDecoderCenter:decoderCenter];
...
- (XLDErr)openFile:(NSString *)file
{
	...
	discLayout = [[XLDDiscLayout alloc] initWithDecoderCenter:decoderCenter];
...
- (XLDErr)openFiles:(NSArray *)files offset:(xldoffset_t)offset prepended:(BOOL)prepended withMetadata:(NSArray *)metadata aligned:(BOOL)align
{
	...
	for(i=0;i<[files count];i++) {
		id <XLDDecoder> decoder = [decoderCenter preferredDecoderForFile:[files objectAtIndex:i]];
		if(!decoder) continue;
...
- (void)setDecoderCenter:(XLDecoderCenter *)center
{
	if(decoderCenter) [decoderCenter release];
