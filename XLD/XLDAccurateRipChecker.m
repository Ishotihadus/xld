...
- (id)init
{
	...
	[super init];
	if([NSThread isMultiThreaded]) [NSBundle loadNibNamed:@"ARChecker" owner:self];
...
- (void)dealloc
{
	...
	if(analyzer) XLDGainAnalyzer_Destroy(&analyzer);
	[super dealloc];
}
...
- (void)startCheckingForFile:(NSString *)path withDecoder:(id)decoderObj
{
	...
	if(([(id <XLDDecoder>)decoder samplerate] != 44100) || ([(id <XLDDecoder>)decoder channels] != 2)) {
		fprintf(stderr,"error: not a CD format (44.1 kHz/Stereo)\n");
		[decoder closeFile];
		[delegate accurateRipCheckDidFinish:self];
		return;
	}
...
- (void)startReplayGainScanningForFile:(NSString *)path withDecoder:(id)decoderObj
{
	...
	if(mode == XLDGainAnalyzerModeReplayGain && [(id <XLDDecoder>)decoder channels] != 2) {
		[decoder closeFile];
		[delegate accurateRipCheckDidFinish:self];
		return;
	}
	
	analyzer = XLDGainAnalyzer_Init([(id <XLDDecoder>)decoder samplerate], [(id <XLDDecoder>)decoder channels], mode);
	...
	running = YES;
	if([NSThread isMultiThreaded]) [NSThread detachNewThreadSelector:@selector(scanReplayGain) toTarget:self withObject:nil];
	else [self scanReplayGain];
}
...
static void printProgress(double percent, int currentTrack, int totalTracks)
{
	fprintf(stderr,"\r|");
	int i;
	for(i=0;i<20;i++) {
	...
	fprintf(stderr,"| %3d%% (Track %d/%d)",(int)percent,currentTrack,totalTracks);
	fflush(stderr);
}
	...
	do {
		...
		if(elapsed1 > 0.25) {
			...
			if([NSThread isMultiThreaded])
				...
				printProgress(percent, currentTrack, trackNumber);
			tv1 = tv2;
		}
		...
		if(currentTrack > trackNumber) {
			...
			if([NSThread isMultiThreaded])
				...
				printProgress(percent, trackNumber, trackNumber);
				fprintf(stderr,"\ndone.\n");
			}
			break;
		}
...
- (NSString *)logStrForReplayGainScanner
{
	...
	[out appendString:@"    ---------------------------------------------------------\n"];
	for(i=0;i<trackNumber;i++) {
		[out appendString:[NSString stringWithFormat:@"      % 3d  | %@ | %@ |   % 7lld    |  % 7lld   \n",i+1,framesToMSFStr(results[i].index,[decoder samplerate]),framesToMSFStr(results[i].length,[decoder samplerate]),results[i].index/588,(results[i].index+results[i].length)/588-1]];
	}
	[out appendString:@"\n"];
...
- (NSString *)logStrForSingleReplayGainScanner
{
	if(!results) return nil;
	if(!running) return nil;
	double albumGain, albumPeak;
	...
	[out appendString:@"XLD ReplayGain scanning logfile\n\n"];
	[out appendFormat:@"%@\n",[decoder srcPath]];
	[out appendString:[NSString stringWithFormat:@"    Gain                   : %.2f dB\n",albumGain]];
	[out appendString:[NSString stringWithFormat:@"    Peak                   : %f\n",albumPeak]];
	[out appendString:[NSString stringWithFormat:@"    CRC32 hash             : %08X\n",crc32_global^0xFFFFFFFF]];
	[out appendString:[NSString stringWithFormat:@"    CRC32 hash (skip zero) : %08X\n\n",crc32_eac_global^0xFFFFFFFF]];
	[out appendString:@"End of status report\n"];
