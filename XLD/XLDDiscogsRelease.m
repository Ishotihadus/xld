...
- (id)initWithReleaseID:(NSString *)releaseid totalTracks:(int)totalTracks totalSectors:(int)sectors
{
	...
	NSMutableArray *tracks = [NSMutableArray arrayWithArray:[rel nodesForXPath:@"./tracklist" error:nil]];
	NSString *alternateDiscTitle = nil;
	NSString *currentAlternateDiscTitle = nil;
	NSMutableDictionary *titlePrefixes = [NSMutableDictionary dictionary];
	int i,j;
	int currentTrack = 0;
	int totalSeconds = 0;
	BOOL match = NO;
	for(i=0;i<[tracks count];i++) {
		...
		NSString *titlePrefix = [titlePrefixes objectForKey:[NSNumber numberWithInt:i]];
		...
		if(![position isEqualToString:@""]) {
			...
			NSString *type = [[[[tracks objectAtIndex:i] nodesForXPath:@"./type_" error:nil] objectAtIndex:0] stringValue];
			if([type isEqualToString:@"index"]) {
				NSArray *subtracks = [[tracks objectAtIndex:i] nodesForXPath:@"./sub_tracks" error:nil];
				if(subtracks) {
					[tracks insertObjects:subtracks atIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(i+1, [subtracks count])]];
					if([subtracks count] > 1 && title && ![title isEqualToString:@""]) {
						int j;
						for(j=i+1;j<i+1+[subtracks count];j++) {
							[titlePrefixes setObject:title forKey:[NSNumber numberWithInt:j]];
						}
					}
				}
		...
		if(![title isEqualToString:@""]) {
			if(titlePrefix) {
				title = [NSString stringWithFormat:@"%@ - %@", titlePrefix, title];
			}
