...
- (IBAction)beginDecode:(id)sender
{
	...
	NSArray *trackList;
	id resultObj = nil;
	XLDCDDARippingSession *rippingSession = nil;
	id cueParser = [discView cueParser];
	...
	if([NSStringFromClass([decoder class]) isEqualToString:@"XLDCDDARipper"]) {
		rippingSession = [XLDCDDARippingSession createSessionForDevice:[cueParser fileToDecode]];
		if([rippingSession open] == -1) {
			NSRunCriticalAlertPanel(LS(@"error"), LS(@"Device is busy"), @"OK", nil, nil);
			[rippingSession destroy];
			return;
		}
		...
		[rippingSession setMaxRippingSpeed:[[o_driveSpeedControl selectedItem] tag]];
		
		resultObj = [[XLDCDDAResult alloc] initWithTrackNumber:[[cueParser trackList] count]];
		[rippingSession setResult:resultObj];
	...
	if(![self canHandleOutputForDecoder:decoder]) {
		...
		if(rippingSession) [rippingSession destroy];
	...
	for(i=0;i<[trackList count];i++) {
		...
		if([o_addiTunes state] == NSOnState) {
			...
			[task setiTunesLib:iTunesLibName withAppName:musicAppName];
		}
		
		if(rippingSession) {
			driveIsBusy = YES;
			[task setCDDARippingSession:rippingSession];
			if(([o_testBeforeCopy state] == NSOnState) && (([[o_testType selectedCell] tag] == 0) || ![resultObj accurateRipDB] || ![[resultObj accurateRipDB] hasValidDataForTrack:i+1])) {
				...
				if(testFlag) {
					...
					[testTask setiTunesLib:nil withAppName:musicAppName];
					[testTask setCDDARippingSession:rippingSession];
...
- (IBAction)showWindow:(id)sender
{
	if([sender tag] == 0) [player showPlayer];
		...
		[o_logWindow makeKeyAndOrderFront:nil];
		/* this was needed in 11.0 beta - I guess no longer needed
		[[o_logWindow toolbar] validateVisibleItems];
		*/
	}
}
...
- (void)updateCDDAListAndMount:(NSString *)device
{
	...
	if(device) [self makeDriveOffsetList];
...
- (IBAction)donate:(id)sender
{
	...
	if([[languages objectAtIndex:0] isEqualToString:@"ja"] || [[languages objectAtIndex:0] isEqualToString:@"ja-JP"]) {
...
- (id)init
{
	...
	serverList = [[NSMutableArray alloc] initWithObjects:@"gnudb.gnudb.org",@"freedbtest.dyndns.org",nil];
...
- (void)loadPrefsFromDictionary:(id)pref
{
	...
	if(obj=[pref objectForKey:@"CDDBServerList"]) {
		NSMutableArray *mutableArr = [obj mutableCopy];
#if MAC_OS_X_VERSION_MAX_ALLOWED >= 1050
		NSUInteger idx = [mutableArr indexOfObject:@"freedb.freedb.org"];
#else
		unsigned int idx = [mutableArr indexOfObject:@"freedb.freedb.org"];
#endif
		if(idx != NSNotFound) {
			[mutableArr replaceObjectAtIndex:idx withObject:@"gnudb.gnudb.org"];
		}
		[serverList removeAllObjects];
		[serverList addObjectsFromArray:mutableArr];
		[o_cddbServer setNumberOfVisibleItems:[mutableArr count]];
		[o_cddbServer reloadData];
	}
...
- (void)readPreGapOfDisc:(NSString *)volume
{
	...
	int i;
	XLDCDDARippingSession *rippingSession = [XLDCDDARippingSession createSessionForDevice:[NSString stringWithUTF8String:statDisc.f_mntfromname]];
	if([rippingSession open] != -1) {
		xld_cdread_t *cdread = [rippingSession descriptor];
		driveIsBusy = YES;
		if(cdread->product) {
...
- (void)finishedReadingPregapWithTrackData:(NSArray *)args
{
	...
	XLDCDDARippingSession *rippingSession = [XLDCDDARippingSession sessionForDevice:[path UTF8String]];
...
- (void)makeDriveOffsetList
{
	...
	int i;
	for(i=[o_offsetList numberOfItems]-1;i>0;i--) {
		[o_offsetList removeItemAtIndex:i];
	}
	...
	if(!found) {
		...
		[o_offsetList setTarget:nil];
		[o_offsetList setAction:nil];
	}
	else {
...
- (void)applicationDidFinishLaunching: (NSNotification *)notification
{
	...
	if([[languages objectAtIndex:0] isEqualToString:@"ja"] || [[languages objectAtIndex:0] isEqualToString:@"ja-JP"]) {
		...
		[o_cuesheetEncodings selectItemWithTitle:[NSString localizedNameOfStringEncoding:NSUTF8StringEncoding]];
	}
	
#if MAC_OS_X_VERSION_MAX_ALLOWED >= 101500
	if(floor(NSAppKitVersionNumber) > 1894) {
		[o_prefPane setToolbarStyle:NSWindowToolbarStylePreference];
	}
#endif
	NSToolbar*  toolbar;
	...
	if(XLDDarkModeSupportEnabled) {
		...
		[o_pluginOptionContentView setWantsLayer:YES];
		[[o_queryResultPane contentView] setWantsLayer:YES];
	}
#endif
...
#endif
	{
		NSString *customMusicAppName = [defs stringForKey:@"XLDCustomMusicAppName"];
		if(floor(NSAppKitVersionNumber) < 1862) {
			musicAppName = @"iTunes";
		}
		else {
			musicAppName = @"Music";
			if([defs boolForKey:@"XLDForceUseiTunesLibrary"] == YES) musicAppName = @"iTunes";
		}
		if(customMusicAppName && ![customMusicAppName isEqualToString:@""]) {
			musicAppName = [customMusicAppName retain];
		}
	}
