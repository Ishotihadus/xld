...
- (void)setupTagDataWithTrack:(id)track andEncoderAttr:(NSString *)encoderAttr
{
	...
	/* version strings */
	if (XLDAacOutput2_EncoderInfoBaseString)
	{
		added = YES;
		NSString *str = [NSString stringWithFormat:@"%@, %@",XLDAacOutput2_EncoderInfoBaseString,encoderAttr];
		atomID[0] = 0xa9;
...
- (BOOL)openFileForOutput:(NSString *)str withTrackData:(id)track
{
	...
	if(ExtAudioFileCreateWithURL((CFURLRef)url, kAudioFileM4AType, &outputFormat, NULL, kAudioFileFlags_EraseFile, &file) != noErr) {
		NSLog(@"ExtAudioFileCreateNew failure");
		file = NULL;
		return NO;
	}
