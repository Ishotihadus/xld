...
static int XLDAacOutput2_QTVersion;
...
+ (void)initialize
{
	if(floor(NSAppKitVersionNumber) >= 1862) { /* 10.15 or later, Classic QT is not available anymore */
		XLDAacOutput2_QTVersion = 0x10000000; /* dummy version value */
	} else {
		SInt32 version = 0;
		OSErr err = Gestalt(gestaltQuickTime,&version);
		if(err == noErr) XLDAacOutput2_QTVersion = version;
	}
	NSMutableString *str = [[NSMutableString alloc] initWithFormat:@"X Lossless Decoder %@",[[NSBundle mainBundle] objectForInfoDictionaryKey: @"CFBundleShortVersionString"]];
	if(XLDAacOutput2_QTVersion && XLDAacOutput2_QTVersion < 0x10000000) {
		[str appendFormat:@", QuickTime %d.%d.%d",(XLDAacOutput2_QTVersion>>24)&0xF,(XLDAacOutput2_QTVersion>>20)&0xF,(XLDAacOutput2_QTVersion>>16)&0xF];
	}
	NSDictionary *audioCodecInfo = [[NSDictionary alloc] initWithContentsOfFile:@"/System/Library/Components/AudioCodecs.component/Contents/version.plist"];
	if(audioCodecInfo) {
		NSString *sourceVersion = [audioCodecInfo objectForKey:@"SourceVersion"];
		if(sourceVersion && [sourceVersion length] > 4 && [[NSCharacterSet decimalDigitCharacterSet] isSupersetOfSet:[NSCharacterSet characterSetWithCharactersInString:sourceVersion]]) {
			NSMutableString *versionString = [[NSMutableString alloc] init];
			unsigned int minorVersionLength = 4;
			unsigned int minorVersionDigits = 2;
			unsigned int numMinorVersionDelimiter = 2;
			if([sourceVersion length] > 12) {
				minorVersionLength = 12;
				minorVersionDigits = 3;
				numMinorVersionDelimiter = 4;
			}
			@try {
				NSRange range = NSMakeRange(0, [sourceVersion length] - minorVersionLength);
				[versionString appendString:[sourceVersion substringWithRange:range]];
				range.location = range.length;
				range.length = minorVersionDigits;
				int i;
				unsigned int effectiveLength;
				for(i=0;i<numMinorVersionDelimiter;i++,range.location+=minorVersionDigits) {
					int subVersion = [[sourceVersion substringWithRange:range] intValue];
					[versionString appendFormat:@".%d",subVersion];
					if(i == 0 || subVersion > 0) effectiveLength = [versionString length];
				}
				[str appendFormat:@", AudioCodecs %@",[versionString substringToIndex:effectiveLength]];
			} @catch (NSException *e) {
			} @finally {
				[versionString release];
			}
		}
		[audioCodecInfo release];
	}
	XLDAacOutput2_EncoderInfoBaseString = str;
}

+ (BOOL)canLoadThisBundle
{
	if (floor(NSAppKitVersionNumber) <= NSAppKitVersionNumber10_3 || XLDAacOutput2_QTVersion < 0x07210000) {
		return NO;
	}
	return YES;
}
...
- (id)init
{
	...
	if(XLDAacOutput2_QTVersion < 0x07630000 || floor(NSAppKitVersionNumber) <= NSAppKitVersionNumber10_4) {
		isSBRAvailable = NO;
	}
